...
type Splice n m = HeistT n m (DList (Chunk n))
...
runChildren :: (Monad n, MonadIO m, MonadBaseControl IO m) => Splice n m
...
runNodeList :: (Monad n, MonadIO m, MonadBaseControl IO m) => [X.Node] -> Splice n m
...
lookupSplice :: Monad m => Text -> HeistT n m (Maybe (Splice n m))
lookupSplice nm = do
...
runNode :: (Monad n, MonadIO m, MonadBaseControl IO m) => X.Node -> Splice n m
runNode node = localParamNode (const node) $ do
    ...
    (res, hs'') <- lift $ catches (compileIO hs')
                     [ Handler (\(ex :: CompileException) -> liftIO $ throwIO ex)
...
-- load time.
subtreeIsStatic :: MonadIO m => X.Node -> HeistT n m Bool
...
compileNode :: (Monad n, MonadIO m, MonadBaseControl IO m) => X.Node -> Splice n m
...
------------------------------------------------------------------------------
getAttributeSplice :: MonadIO m => Text -> HeistT n m (DList (Chunk n))
...
getAttributeSplice2 :: (Monad n, MonadIO m)
                    => Text
...
bindSplices :: Splices (Splice n m)  -- ^ splices to bind
            -> HeistState n m        -- ^ source state
            -> HeistState n m
bindSplices ss hs =
...
-- of a RuntimeSplice.
pureSplice :: (Monad n, Monad m) => (a -> Builder) -> RuntimeSplice n a -> Splice n m
...
withSplices :: (Monad n, Monad m)
            => Splice n m
            -- ^ Splice to be run
            -> Splices (RuntimeSplice n a -> Splice n m)
